...
public class CompresorHuffman {
    ...
    //EL arbol de Huffman construido a partir del ULTIMO texto comprimido; se necesita conservarlo, porque para descomprimir hace falta el MISMO arbol que se uso para comprimir
    private ArbolHuffman arbol;

    //CONSTRUCTOR: prepara las dos piezas internas, todavia sin ningun texto procesado
    ...
    /**
     * Comprime un texto completo, construyendo el arbol de Huffman especifico para ese texto
    ...
    public String comprimir(String texto){
        ...
        //PASO 3: construir el arbol de Huffman (y con el, la tabla de codigos) a partir de esa cola
        //Se guarda en el atributo arbol, porque descomprimir() lo va a necesitar despues
        arbol = new ArbolHuffman();
        arbol.construir(cola);

        //PASO 4: recorrer el texto original y sustituir cada caracter por su codigo en bits
        //Usamos StringBuilder porque concatenar String con "+" en un ciclo es muy ineficiente
    ...
    /**
     * Descomprime una cadena de bits usando el arbol generado en la ULTIMA llamada a comprimir()
    ...
    public String descomprimir(String bits){
        ...
        //CASO LIMITE ESPECIAL: el texto original tenia UN SOLO caracter distinto; En ese caso la raiz del arbol es directamente una hoja (no tiene hijos), asi que no se puede "avanzar" por el arbol; cada bit representa una repeticion de ese unico caracter
     ...
     * @return el tamanio del texto original en bits, asumiendo 8 bits por caracter (como ASCII estandar)
     */
    public int getTamanioOriginalBits(String texto){
        ...
        //Se compara contra 8 bits por caracter porque es la referencia estandar (ASCII); asi el porcentaje de ahorro tiene sentido para quien lo vea en la interfaz
        return texto.length() * 8;
    }//Cierre getTamanioOriginalBits

    /**
     *@param bits la cadena de bits ya comprimida
     *@return el tamanio del texto comprimido en bits (cadena caracter '0' o '1' de la cadena es un bit)
     */
    ...
    public double getPorcentajeAhorro(String texto, String bits){
        ...
        //CASO LIMITE: si el original media 0 bits, no hay porcentaje que calcular (evita dividir entre cero)
        if (original == 0){
            return 0;
        }//Cierre if
    ...
    //GETTER: le da acceso a la interfaz grafica al arbol ya construido, por si se quiere mostrar informacion adicional (como la tabla de codigos o, mas adelante, un dibujo del arbol)
